...
export async function sendPush(env, subscription, payload) {
  ...
  const response=await fetch(request.endpoint,{method:'POST',headers,body});
  let reason=null;
...
export async function dispatchNotes(env, now=Date.now(), targetId=null) {
  ...
  for(const job of jobs) {
    ...
    let status,reason;
    try {({status,reason}=await sendPush(env,{endpoint:current.endpoint,keys:{p256dh:current.p256dh,auth:current.auth}},{account:current.user_id,id:current.id,title:'Two Lovebugs ♡',body,url:current.note_id?`/notes?note=${current.note_id}`:'/notes',unread:count.n}));} catch(error){status=0;const detail=String(error?.message||'').replace(/https?:\/\/[^\s]+/g,'push-service').replace(/[^A-Za-z0-9 .:_-]/g,'').slice(0,120);reason=(error?.name==='TimeoutError'?'PushServiceTimeout':error?.name==='TypeError'?'WorkerTransportError':'TransportOrConfigurationError')+(detail?': '+detail:'');}
