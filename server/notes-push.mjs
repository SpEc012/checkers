...
export async function sendPush(env, subscription, payload) {
  const request=webpush.generateRequestDetails(subscription,JSON.stringify(payload),{TTL:3600,vapidDetails:{subject:env.VAPID_SUBJECT,publicKey:env.VAPID_PUBLIC_KEY,privateKey:env.VAPID_PRIVATE_KEY}});
...
export async function dispatchNotes(env, now=Date.now()) {
  const db=env.DB;
  await publishDueNotes(env,now);
  if(!pushConfigured(env)) return;
  const {results:jobs}=await db.prepare(`SELECT id FROM ln_outbox WHERE status IN ('pending','retry','sending') AND next_at<=? ORDER BY next_at LIMIT 30`).bind(now).all();
  for(const job of jobs) {
    ...
    let status;
    try {status=await sendPush(env,{endpoint:current.endpoint,keys:{p256dh:current.p256dh,auth:current.auth}},{account:current.user_id,id:current.id,title:'Two Lovebugs ♡',body,url:current.note_id?`/notes?note=${current.note_id}`:'/notes',unread:count.n});} catch {status=503;}
    if(status===404 || status===410) {await db.prepare('DELETE FROM ln_push WHERE id=?').bind(current.subscription_id).run();continue;}
    const succeeded=status>=200 && status<300;
    const nextStatus=succeeded?'accepted':claimed.attempts>=8 || (status>=400 && status<500 && status!==429)?'failed':'retry';
    await db.prepare('UPDATE ln_outbox SET status=?,next_at=? WHERE id=? AND lease=?').bind(nextStatus,now+Math.min(3600000,30000*2**claimed.attempts),job.id,lease).run();
  }
