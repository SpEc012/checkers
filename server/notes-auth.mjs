...
export const user = sqliteTable('ln_user',{id:text('id').primaryKey(),name:text('name').notNull(),email:text('email').notNull().unique(),username:text('username').unique(),displayUsername:text('display_username'),emailVerified:integer('email_verified',{mode:'boolean'}).notNull().default(false),image:text('image'),createdAt:time('created_at').notNull(),updatedAt:time('updated_at').notNull()});
...
export function createNotesAuth(env) {
  ...
  return betterAuth({
    ...
    emailAndPassword:{enabled:true,requireEmailVerification:true,minPasswordLength:8,maxPasswordLength:128},
    ...
    rateLimit:{enabled:true,storage:'database',window:60,max:60,customRules:{'/email-otp/send-verification-otp':{window:60,max:3},'/sign-in/email-otp':{window:60,max:6},'/sign-in/username':{window:60,max:6}}},
    plugins:[username({minUsernameLength:3,maxUsernameLength:30}),emailOTP({otpLength:6,expiresIn:600,allowedAttempts:5,storeOTP:'hashed',async sendVerificationOTP({email,otp}) {
