...
export function createArena(container,label){
 ...
 const scene=new THREE.Scene(),camera=new THREE.PerspectiveCamera(36,1,.1,50);camera.position.set(0,1,11);camera.lookAt(0,.3,0);
 ...
 const resize=new ResizeObserver(()=>{const w=container.clientWidth,h=container.clientHeight;renderer.setSize(w,h,false);camera.aspect=w/h;camera.updateProjectionMatrix()});resize.observe(container);
 ...
 renderer.setAnimationLoop((time)=>{if(document.hidden||!container.checkVisibility())return;const elapsed=(performance.now()-start)/1000,shake=revealed&&!reduced.matches&&elapsed<1.8,blend=revealed?(reduced.matches?1:Math.min(1,Math.max(0,(elapsed-1.8)*4))):0;
 hands.forEach((h,i)=>{pose(h,picks?.[i===0?'rose':'cream']||'rock',blend);h.root.position.y=-.25+(shake?Math.sin(elapsed*Math.PI*3.33)*.22:reduced.matches?0:Math.sin(time*.0015+i)*.035);h.root.rotation.x=shake?Math.sin(elapsed*Math.PI*3.33)*.18:0});label.textContent=shake?['Rock…','Paper…','Scissors…'][Math.min(2,Math.floor(elapsed/.6))]:revealed?'Shoot!': 'Two hearts. One showdown.';renderer.render(scene,camera);
 });
 return {update(s){if(s.key===key)return;key=s.key;if(s.revealed&&!revealed)start=performance.now();revealed=s.revealed;picks=s.picks;}};
}
