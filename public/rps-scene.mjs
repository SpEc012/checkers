...
import {throwFrame,armPose} from './match-effects.mjs';
export function createArena(container,label){
 ...
 function hand(x,color,mirror){const pivot=new THREE.Group();scene.add(pivot);const root=new THREE.Group();root.position.set(0,1.25,0);root.rotation.set(.35,0,0);if(mirror>0)root.scale.x=-1;pivot.add(root);const skin=new THREE.MeshStandardMaterial({color,roughness:.64,metalness:0});
 ...
 hands.push({root,pivot,fingers,thumb,x,mirror});}
 ...
 hands.forEach((h,index)=>{const choice=snapshot.picks?.[index?'cream':'rose']||'rock';h.fingers.forEach(({base,joints},i)=>{const open=choice==='paper'||choice==='scissors'&&i<2;const bends=open?[0,.08,.05]:[1.35,1.65,1.25];joints.forEach((joint,j)=>joint.rotation.x=[1.35,1.65,1.25][j]+(bends[j]-[1.35,1.65,1.25][j])*blend);base.rotation.z=blend*(choice==='scissors'&&i<2?(i===0?.18:-.18):choice==='paper'?(1.5-i)*.06:0)});h.thumb.rotation.x=1.1-blend*(choice==='paper'?1.25:0);h.thumb.rotation.z=.85+blend*(choice==='paper'?.25:0);const motion=snapshot.revealed?f:{swing:.1,reach:0};const pose=armPose(index?'cream':'rose',motion);h.pivot.position.set(pose.x,pose.y,0);h.pivot.rotation.z=pose.angle;h.root.position.y=pose.extension;h.root.rotation.x=.35-.18*blend;});
