...
self.addEventListener('push',event=>event.waitUntil((async()=>{
 ...
 let session;try{const r=await fetch('/api/notes/me',{credentials:'include',cache:'no-store'});if(r.ok)session=await r.json();else if(r.status===401){await self.registration.showNotification('Two Lovebugs ♡',{body:'Open your postbox to sign in.',tag:'signed-out',data:{url:'/notes'}});return;}}catch{}
 ...
 if(self.navigator.setAppBadge){const n=session?.unread??data.unread??0;if(n)await self.navigator.setAppBadge(n);else await self.navigator.clearAppBadge();}
})()));
