...
let assets={};for(const [file,type] of [['index.html','text/html; charset=utf-8'],['style.css','text/css'],['app.mjs','text/javascript'],['engine.mjs','text/javascript'],['arcade.mjs','text/javascript']])assets['/'+file]={body:readFileSync('public/'+file,'utf8'),type};
const engine=readFileSync('public/engine.mjs','utf8').replaceAll('export function','function');
const arcade=readFileSync('public/arcade.mjs','utf8').replace("import {initial} from './engine.mjs';",'').replaceAll('export function','function').replaceAll('export const','const');
const api=readFileSync('server/api.mjs','utf8').replace("import { initial, apply } from '../public/engine.mjs';",'').replace("import {newGame,dropHeart,placeTile,drawingAction,publicGame,gameNames} from '../public/arcade.mjs';",'').replace('export async function api','async function api');
