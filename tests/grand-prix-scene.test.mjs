...
import {gpNew,gpAction,gpAdvance,GP_TRACKS} from '../public/grand-prix.mjs';
...
const sandbox={RacePrediction,structuredClone,THREE:{...Three,WebGLRenderer:Renderer},mergeGeometries,gpNew,gpAction,gpAdvance,GP_TRACKS,URLSearchParams,location:{search:'',origin:'https://game.test'},document,window:{addEventListener(){}},localStorage:{getItem:k=>storage.get(k),setItem:(k,v)=>storage.set(k,v)},matchMedia:()=>({matches:false}),devicePixelRatio:2,innerWidth:390,innerHeight:844,performance,requestAnimationFrame(){},setTimeout:()=>0,setInterval:()=>0,console};sandbox.globalThis=sandbox;
...
console.log('Grand Prix scene startup, garage, three courses, simple kart geometry without combat passed (GPU rendering not exercised).');
